...
object DataOps {
  ...
  def DeleteData(dbClient: AmazonSimpleDBClient, domain: String, item: String): Unit = {
    println(s"正在删除数据: domain -> $domain,  item -> $item")
  ...
  def WritePriamAttrs(dbClient: AmazonSimpleDBClient): Unit = {
    ...
    attrs.add(new ReplaceableAttribute("appId", CassCluster, false))
    ...
    attrs.add(new ReplaceableAttribute("value" , "/etc/init.d/dse start", false))
    ...
    attrs.add(new ReplaceableAttribute("value" , "/etc/init.d/dse stop", false))
